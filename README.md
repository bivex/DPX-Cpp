...
# 5. Trace Forward Data Flow for a specific variable (Data Flow Out)
uv run pattern-detector dataflow transformedData --path ./src

# 6. Trace Backward Data Flow Slice (Data Flow In)
uv run pattern-detector dataflow reportValue --path ./src --direction in

# 7. Trace Relationship Path connecting two entities
uv run pattern-detector dataflow transformedData --to reportValue --path ./src

# 8. Export Data Flow Graph to Mermaid.js or JSON
